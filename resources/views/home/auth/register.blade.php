    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#get-code').click(function(){
                var target = "{{ route('home.register.send') }}";
                var mobile = $('#username').val();
                if(!mobile){
                    showError('请输入你要验证的手机号码','username');
                    return false;
                }
                if (!/^1[34578]{1}\d{9}$/.test(mobile)){
                    showError('手机号码格式错误','username');
                    return false;
                }
                var resend = "{{$resend}}";
            ...
            $('#code').blur(function(){
                var code = $(this).val();
                var target = "{{ route('home.register.verify') }}";
                var mobile = $('#username').val();
                if(!code){
                    showError('请输入验证码','username');
                    return false;
                }
                var _token = "{{csrf_token()}}";
                ...
                <div class="panel-body">
                    <form class="form-horizontal data-form" action="{{ route('home.register.post') }}" method="POST" >
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                {!! Form::homeRadios("type", $members, 1,array('autocomplete'=>'off')) !!}
                            </div>
                        </div>
